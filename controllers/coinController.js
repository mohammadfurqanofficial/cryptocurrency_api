...
exports.getCoinHistory = async (req, res) => {
  ...
  try {
    // Find the specific favorite coin based on the coin ID
    ...
    const coinHistory = await CoinHistory.find({ coinId: favoriteCoin.coinId });
    ...
    // Respond with the favorite coin details and its history
    res.status(200).json({
      message: "Coin history retrieved successfully",
      coin: {
